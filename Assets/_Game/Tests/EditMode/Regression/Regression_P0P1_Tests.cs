...
namespace SeasonalBastion.Tests.EditMode
{
    public sealed class Regression_P0P1_Tests
    {
        ...
        public void AmmoResupply_ZeroAmmo_AlwaysTriggersJob_WhenStorageHasAmmo()
        {
            ...
            var armory = CreateConstructedBuilding(world, data, "bld_armory_t1", new CellPos(1, 1), level: 1, isArmory: true, workRoles: WorkRoleFlags.Armory);
        ...
        public void AmmoResupply_MultipleTowers_PrioritizesLowestAmmoThenTowerId()
        {
            ...
            var armory = CreateConstructedBuilding(world, data, "bld_armory_t1", new CellPos(1, 1), level: 1, isArmory: true, workRoles: WorkRoleFlags.Armory);
            CreateNpc(world, armory, new CellPos(1, 2));
            var towerA = CreateTower(world, new CellPos(10, 10), ammo: 3, ammoCap: 20);
        ...
        [Test]
        public void AmmoResupply_JobBoardRecovered_RecreatesJob_WhenQueueIsEmptyButTowerNeedsAmmo()
        {
            var bus = new TestEventBus();
            var world = new WorldState();
            var data = new TestDataRegistry();
        ...
        private static BuildingId CreateConstructedBuilding(WorldState world, TestDataRegistry data, string defId, CellPos anchor, int level, bool isArmory, WorkRoleFlags workRoles)
        {
            data.Add(new BuildingDef
            {
                DefId = defId,
