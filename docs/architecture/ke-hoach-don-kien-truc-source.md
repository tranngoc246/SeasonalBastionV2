...
- [x] Đã bóc tiếp `NpcIdleRoamService`: service này không còn giữ `GameServices`, mà nhận `IAgentMover`, `IDataRegistry`, `IGridMap`, `IWorldState`; các call-site `EntryCellUtil` trong idle roaming cũng đã chuyển sang overload dependency hẹp.
