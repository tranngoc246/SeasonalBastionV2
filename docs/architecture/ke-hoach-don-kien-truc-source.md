...
- [x] Đã bóc tiếp `NpcIdleRoamService`: service này không còn giữ `GameServices`, mà nhận `IAgentMoverRuntime`, `IDataRegistry`, `IGridMap`, `IWorldState`; các call-site `EntryCellUtil` trong idle roaming cũng đã chuyển sang overload dependency hẹp.
- [x] Đã chốt pass compile-fix sau refactor boundary:
