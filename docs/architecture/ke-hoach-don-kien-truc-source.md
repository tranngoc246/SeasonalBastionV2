...
- [~] Cụm `Build*` hiện đã có dependency boundary rõ hơn rõ rệt; phần còn lại đáng cân nhắc chủ yếu là rà tiếp các helper/executor còn giữ `GameServices` vì lý do path/build interaction, hoặc dừng cụm này ở checkpoint hiện tại để chuyển mặt trận.
